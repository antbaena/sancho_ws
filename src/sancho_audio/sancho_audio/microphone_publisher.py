...
class MicrophoneCapturer(Node):
    ...
    def __init__(self, device_index, chunk_size):
        ...
        self.get_logger().info(
            "Inicializando microfono en el indice: " + str(device_index)
        )
        ...
        self.publisher_stereo = self.create_publisher(
            ChunkStereo, "AudioRaw/microphone/stereo", 10
        )
        self.publisher_mono = self.create_publisher(
            ChunkMono, "AudioRaw/microphone/mono", 10
        )
        self.setup_microphone(
            self.device_index, self.sample_rate, self.num_channels, self.chunk_size
        )
            ...
            try:
                ...
                if self.num_channels == 1:
                    ...
                    chunk_mono.chunk_mono = audio_mix[
                        ::2
                    ]  # Probar a hacer algun metodo de mono to stereo

                    self.publisher_stereo.publish(chunk_stereo)

                if first:
                    self.get_logger().info(
                        "Stereo audio frame published (if num channels is 1 or 2)"
                    )
                    first = False
...
def main(args=None):
    ...
    microphone_capturer = MicrophoneCapturer(
        device_index=microphone_index, chunk_size=1024
    )
