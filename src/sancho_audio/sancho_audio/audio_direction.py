...
class AudioDirection(Node):
    ...
    def __init__(
        self, sample_rate=48000, distance_mics=0.1225, direction_chunk_seconds=1
    ):
        ...
        self.subscription = self.create_subscription(
            ChunkStereo, "AudioRaw/microphone/stereo", self.listener_callback, 10
        )
        self.direction_client = self.create_service(
            AudioLocation, "audio_location", self.audio_location
        )
        self.publisher_human_voice_pan = self.create_publisher(
            String, "audio/human_voice_pan", 1
        )
    ...
    def audio_location(self, request, response, intensity_threshold=300):
        ...
        if (
            self.audio_average_intensity(
                np.array(
                    AudioSegment.from_wav(temp_wav_path).get_array_of_samples(),
                    dtype=np.int16,
                )
            )
            > intensity_threshold
        ...
        if has_speech:
            ...
            if longest_segment:
                ...
                _, _, best_shift = determine_audio_location(
                    microphone_left=np.asarray(audio_left_segment),
                ...
                sin_degree = (
                    1 if sin_degree > 1 else (-1 if sin_degree < -1 else sin_degree)
                )

                angle_rad = np.arcsin(sin_degree)
                depth = np.sqrt(
                    np.abs(
                        (self.distance_mics * self.distance_mics)
                        - (
                            (time_variation * self.speed_sound)
                            * (time_variation * self.speed_sound)
                        )
                    )
                )
        ...
        self.publisher_human_voice_pan.publish(
            String(
                data=json.dumps(
                    {"intensity": response.intensity, "angle_rad": response.angle_rad}
                )
            )
        )
    ...
    def listener_callback(self, msg):
        ...
        self.audio_right = self.audio_right + list(
            [np.int16(x) for x in msg.chunk_right]
        )
