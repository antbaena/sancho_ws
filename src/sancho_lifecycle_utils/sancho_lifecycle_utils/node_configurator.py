...
from lifecycle_msgs.srv import GetAvailableTransitions, ChangeState
...
class NodeConfigurator(Node):
    def __init__(self):
        ...
        if not self.node_names:
            self.get_logger().error('No se proporcionaron nodos válidos en "node_names".')
            rclpy.shutdown()
            return
    ...
    def _on_timer(self):
        ...
        if not self._pending:
            self.get_logger().info('Todos los nodos han sido configurados. Finalizando node_configurator.')
            rclpy.shutdown()
            return
        ...
        for node_name in list(self._pending):
            ...
            if self.pending_configure_id[node_name] is None:
                if not self.get_in_flight[node_name]:
                    ...
                    if cli_get.service_is_ready():
                        self.get_logger().debug(f'→ Solicitando transiciones de {node_name}')
                        ...
                        self.get_logger().debug(f'Servicio "get_available_transitions" de {node_name} NO listo aún.')
            ...
            if (self.pending_configure_id[node_name] is not None
                ...
                if cli_chg.service_is_ready():
                    ...
                    self.get_logger().debug(f'→ Solicitando ChangeState(configure={tid}) en {node_name}')
                    ...
                    self.get_logger().debug(f'Servicio "change_state" de {node_name} NO listo aún.')
                continue
    ...
    def _on_get_available(self, node_name, future):
        ...
        if resp is None:
            self.get_logger().warning(f'Respuesta nula de GetAvailableTransitions en {node_name}.')
            return
