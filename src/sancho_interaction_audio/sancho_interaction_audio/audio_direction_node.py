...
def compute_angle_from_delay(delay: float,
                             mic_distance: float,
    ...
    def timer_callback(self):
        ...
        for seg in self.buffer:
            ...
            if self.method == 'gcc':
                ...
                tau, _ = gcc_phat(left, right, fs=self.sample_rate,
                                   max_tau=self.mic_distance/343.0)
            ...
            self.get_logger().info(f'Publicado ángulo: {angle:.2f}° (método={self.method})')
        # Clear buffer
        self.buffer.clear()
