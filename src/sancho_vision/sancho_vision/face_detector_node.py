...
class FaceDetectorNode(Node):
    def __init__(self):
        ...
        if self.visualization:
            self.vis_pub = self.create_publisher(Image, detections_topic + "/vis", qos)
    ...
    def _process(self):
        ...
        small = cv2.resize(
            cv_img,
            None,
            fx=self.get_parameter("downscale_factor").value,
        ...
        vis_img = cv_img.copy()
        for (x1, y1, x2, y2), sc in zip(filtered, filtered_scores, strict=False):
            cx = (x1 + x2) / 2.0
            cy = (y1 + y2) / 2.0
            w = x2 - x1
            h = y2 - y1
