...
class FaceTracker(Node):
    def __init__(self):
        ...
        self.create_subscription(FaceArray,
                                 self.face_topic,
                                 self.face_callback,
                                 qos)
    ...
    def camera_info_callback(self, msg: CameraInfo):
        if not self.camera_info_ready:
            ...
            self.cx = msg.k[2]
            self.cy = msg.k[5]
            self.camera_matrix_inv = np.linalg.inv(np.array([[self.fx, 0, self.cx],
                                                             [0, self.fy, self.cy],
                                                             [0, 0, 1]]))
            self.camera_info_ready = True
            self.get_logger().info(f'CameraInfo OK: fx={self.fx:.1f}, fy={self.fy:.1f}, cx={self.cx:.1f}, cy={self.cy:.1f}')
    ...
    def face_callback(self, msg: FaceArray):
        ...
        if self.smoothed_u is None:
            ...
            self.smoothed_u = self.ema_alpha * u + (1 - self.ema_alpha) * self.smoothed_u
            self.smoothed_v = self.ema_alpha * v + (1 - self.ema_alpha) * self.smoothed_v
